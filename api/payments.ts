  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (body.action === 'create') {
        ...
        const valid = (current === 'Pending Approval' && (next === 'Approved for Processing' || next === 'Rejected'))
          || (current === 'Approved for Processing' && (next === 'Payment Made' || next === 'Rejected'));
    ...
    const visible = canProcess ? payments : payments.filter((item) => item.requestedByEmail.toLowerCase() === user.email.toLowerCase());
    return sendJson(res, 200, { payments: visible, canProcess });
  } catch (error: any) {
